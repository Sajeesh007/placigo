...
import { getCollegeProfile, updateCollegeProfile } from "@/services/profile.service"

import CollegeProfileEdit from "@/components/Profile/College/CollegeProfileEdit"
import CollegeLayout from "@/modules/Layout/CollegeLayout"

export default function ProfileCollege() {
    ...
    useEffect(() => {
        user?.id && getCollegeProfile({id:user?.id, setprofileData: setprofileData, seterror: seterror}) 
    }, [user])
    ...
    const onSubmit = async (data) => {
        await updateCollegeProfile({id:user?.id, data: data, seterror: seterror, setloading: setloading})
    }
    ...
    return (
        <div className='flex flex-col space-y-4 items-center pt-4 pb-20 bg-zinc-900'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col space-y-4'>
                ...
                <CollegeProfileEdit register={register} errors={errors}/> 
...
ProfileCollege.getLayout = function getLayout(page) {
    return (
      <CollegeLayout>
            {page}
      </CollegeLayout>
