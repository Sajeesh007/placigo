...
import { addJob, getJobById } from 'services/job.service'
...
export default function EditJobPage() {
    ...
    useEffect(() => {
        router.isReady && getJobById({id: router.query?.jobId, setjobData: setjobData, seterror: seterror, setloading: setloading})
    }, [router.isReady]);
  ...
  return (
    <div className='flex flex-col bg-zinc-900 min-h-screen relative pb-8'>
        ...
        <div className="sticky w-screen h-16 flex justify-center items-center px-3">
            <AiOutlineClose className='w-8 h-8 text-white absolute left-3' onClick={()=> router.push(`/company/jobs/${jobData?.id}`)}/>
...
EditJobPage.getLayout = function getLayout(page) {
    return (
